package com.k2j.bargains.user.service;
...
import com.k2j.bargains.common.exception.GlobalException;
...
import com.k2j.bargains.user.persistence.BargainsUserMapper;
...
public class UserServiceImpl implements UserServiceApi {
    ...
    @Autowired
    BargainsUserMapper userMapper;

    /**
     * @description: 由于需要将一个cookie对应的用户存入第三方缓存中，这里用redis，所以需要引入redis service
     * @author: Sakura
     * @date: 4/8/20
     * @param null:
     * @return: null
     ...
     * @return: com.k2j.bargains.common.result.CodeMsg 状态码
     **/
    @Override
    public CodeMsg register(RegisterVo userModel) {
        ...
        // 检查用户是否注册
        BargainsUser user = this.getBargainsUserByPhone(userModel.getPhone());
        ...
        // 生成bguser对象
        BargainsUser newUser = new BargainsUser();
    ...
    /**
     * @description: 根据 phone 查询秒杀用户信息
     * 对象级缓存
     * 从缓存中查询 BargainsUser 对象，如果 BargainsUser 在缓存中存在，则直接返回，否则从数据库返回
     * @author: Sakura
     * @date: 4/8/20
     * @param phone: 用户手机号码
     * @return: com.k2j.bargains.user.domain.BargainsUser
     **/
    private BargainsUser getBargainsUserByPhone(long phone) {

        // 1. 从redis中获取用户数据缓存
        BargainsUser user = redisService.get(BgUserKeyPrefix.BG_USER_PHONE, "_" + phone, BargainsUser.class);
        ...
        if (user != null)
            redisService.set(BgUserKeyPrefix.BG_USER_PHONE, "_" + phone, user);
        return user;
    }
    ...
    /**
     * @description: 用户登录, 要么处理成功返回true，否则会抛出全局异常
     * 抛出的异常信息会被全局异常接收，全局异常会将异常信息传递到全局异常处理器
     * @author: Sakura
     * @date: 4/8/20
     * @param loginVo: 封装了客户端请求传递过来的数据（即账号密码）（使用post方式，请求参数放在了请求体中，这个参数就是获取请求体中的数据）
    ...
    public String login(@Valid LoginVo loginVo) {
        ...
        BargainsUser user = this.getBargainsUserByPhone(Long.parseLong(mobile));
        ...
        redisService.set(BgUserKeyPrefix.TOKEN, token, user);
        return token;
    }
    ...
    public UserVo getUserByPhone(long phone) {
        ...
        BargainsUser user = userMapper.getUserByPhone(phone);
