package com.k2j.bargains.gateway.goods;
...
import com.k2j.bargains.common.api.goods.vo.GoodsDetailVo;
...
import com.k2j.bargains.gateway.user.UserController;
...
public class GoodsController {
    ...
    /**
     * @description: 因为在redis缓存中不存页面缓存时需要手动渲染，所以注入一个视图解析器，自定义渲染
     * @author: Sakura
     * @date: 4/10/20
     **/
    @Autowired
    ...
    /**
     * @description: 获取 BGUser 对象，并将其传递到页面解析器
     * 从数据库中获取商品信息（包含秒杀信息）
     ...
     * @param response:
     * @param model: 响应的资源文件
     * @param user: 通过自定义参数解析器UserArgumentResolver解析的 BGUser 对象
    ...
    /**
     * @description: 处理商品详情页（页面静态化处理, 直接将数据返回给客户端，交给客户端处理）
     ...
     * @param user:
     * @param goodsId:
     * @return: com.k2j.bargains.common.result.Result<com.k2j.bargains.common.api.goods.vo.GoodsDetailVo>
     **/
    ...
    public Result<GoodsDetailVo> getDetails(UserVo user, @PathVariable("goodsId") long goodsId) {
        ...
        goodsDetailVo.setBargainsStatus(bgStatus);
